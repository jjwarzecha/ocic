...
identity() {
    ...
    echo
    read -p 'What is your username (Email address) ? ' myusername
    echo
    read -rs -p 'What is the corresponding password ?' 'secret'  # use quotes in case of using special characters
    echo
    echo
    read -p 'What is the name of the container ? ' mycontainer
    echo
    ...
    container=$(echo "$mycontainer" | sed 'y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/')
...
cloudauth() {
  auth=$(curl -v -s -X GET -H "X-Storage-User:Storage-${domain}:${username}" -H "X-Storage-Pass:${secret}" https://${domain}.storage.oraclecloud.com/auth/v1.0 --stderr -)
...
download() {
	...
	files=$(curl --get -H "${token}" https://${domain}.storage.oraclecloud.com/v1/Storage-${domain}/${container})
	...
	for i in $(cat files2) ; do
        ...
        echo "$o"
        curl --get -H "${token}" -o "./"$foldername"/$o" https://${domain}.storage.oraclecloud.com/v1/Storage-${domain}/${container}/$i	
    done
        rm files files2
    echo
...
delete() {
	...
	for i in $(cat files2) ; do
    	echo "$i"
        curl -X DELETE -H "${token}" https://${domain}.storage.oraclecloud.com/v1/Storage-${domain}/${container}/$i
	done
    rm files files2
    echo
